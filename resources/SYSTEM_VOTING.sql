...
CREATE TABLE OPTION_TIPO_USUARIO (
...
CREATE PROCEDURE SP_CREATE_PARTIDO (IN NOMBRE VARCHAR(200), IN ESTADO INT)
BEGIN 
	INSERT INTO PARTIDO (PAR_NOMBRE, ESTADO) VALUES (NOMBRE, ESTADO);
END

CREATE PROCEDURE SP_UPDATE_PARTIDO(IN ID INT, IN NOMBRE VARCHAR(200), IN ESTADO INT)
BEGIN
	UPDATE PARTIDO SET PAR_NOMBRE = NOMBRE, ESTADO = ESTADO WHERE PARTIDO.PAR_ID = ID;
END

CREATE PROCEDURE SP_READ_PARTIDOS()
BEGIN
	SELECT * FROM PARTIDO;
END

CREATE PROCEDURE SP_DELETE_PARTIDO (IN NOMBRE VARCHAR(200))
BEGIN
DELETE FROM PARTIDO WHERE PARTIDO.PAR_NOMBRE = NOMBRE;
END

CREATE PROCEDURE SP_CREATE_CANDIDATO (IN DUI VARCHAR(10), IN ID_PARTIDO INT, IN ID_DEP INT, IN ID_MUN INT, IN ID_TIPO INT, IN ESTADO INT)
BEGIN
	INSERT INTO CANDIDATO (CAN_PER_DUI, CAN_PAR_ID, CAN_DEP_ID, CAN_MUN_ID, CAN_TCA_ID, ESTADO) VALUES (DUI, ID_PARTIDO, ID_DEP, ID_MUN, ID_TIPO, 1);
END

CREATE PROCEDURE SP_READ_CANDIDATOS()
BEGIN
	SELECT * FROM CANDIDATO;
END

CREATE PROCEDURE SP_UPDATE_CANDIDATO (IN ID INT, IN DUI VARCHAR(10), IN ID_PARTIDO INT, IN ID_DEP INT, IN ID_MUN INT, IN ID_TIPO INT, IN ESTADO INT)
BEGIN
	UPDATE CANDIDATO SET CAN_PER_DUI = DUI, CAN_PAR_ID = ID_PARTIDO, CAN_DEP_ID = ID_DEP, CAN_MUN_ID = ID_MUN, CAN_TCA_ID = ID_TIPO, ESTADO = ESTADO WHERE CAN_ID = ID;
END

CREATE PROCEDURE SP_DELETE_CANDIDATO(IN ID INT)
BEGIN
	DELETE FROM CANDIDATO WHERE CANDIDATO.CAN_ID = ID;
END
=======
...
>>>>>>> 4bbff1bfd5f9c374bdc0043467a03f065afdcced

CREATE PROCEDURE SP_READ_VOTANTES()
BEGIN
	SELECT * FROM VOTANTE;
END